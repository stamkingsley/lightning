...
export default () => {
  ...
  if (__ITER === 0) {
    try {
      ...
      console.error(
        `VU ${__VU} failed to connect at iteration ${__ITER}:`,
        error,
      );
      sleep(1);
      return; // 跳过当前迭代
  ...
  try {
    getAccountResponse = client.invoke(
      "schema.Lightning/getAccount",
      getAccountRequest,
    );
    ...
    console.error(
      `VU ${__VU} getAccount invoke failed (accountId=${data.accountId}):`,
      error,
    );
  }
  ...
  try {
    increaseResponse = client.invoke(
      "schema.Lightning/increase",
      increaseRequest,
    );
    ...
    console.error(
      `VU ${__VU} increase invoke failed (accountId=${data.accountId}):`,
      error,
    );
  }
  ...
  try {
    decreaseResponse = client.invoke(
      "schema.Lightning/decrease",
      decreaseRequest,
    );
    ...
    console.error(
      `VU ${__VU} decrease invoke failed (accountId=${data.accountId}):`,
      error,
    );
  }
